...
Directory Processing:
    python trim_pcap.py -d /path/to/pcap/files
    python trim_pcap.py -d /path/to/source/pcaps /path/to/destination
    python trim_pcap.py -d ./source_dir ./output_dir -p "4500" --verbose
    python trim_pcap.py -d /data/pcaps /data/processed --batch-size 2000 --enable-threading
...
5. Testing Large Files (Dry Run):
   python trim_pcap.py test_large.pcap /tmp/output.pcap \
     --dry-run \
     --batch-size 3000 \
     --verbose \
     --enable-threading
...
DIRECTORY PROCESSING EXAMPLES:
  %(prog)s -d /path/to/pcap/files
  %(prog)s -d /path/to/pcap/files /path/to/output
  %(prog)s -d ./source_pcaps ./processed_pcaps -p "4500" --verbose
  %(prog)s -d /data/captures /data/cleaned --batch-size 2000 --enable-threading
  %(prog)s -d ./network_logs --ascii -p "HTTP" --dry-run

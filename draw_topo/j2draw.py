...
class DeviceInfo:
    ...
    def extract_ip_part(self, ip_info: str) -> str:
        """Extract just the IP part from interface IP information"""
        if not ip_info:
            return ""
...
class TopologyBuilder:
    ...
    def _extract_interfaces_section(self, content: str) -> str:
        ...
        # We need to make sure we find the main interfaces section, not the one in protocols lldp
        brace_count = 0
        for i, line in enumerate(lines):
            ...
            # Look for 'interfaces {' at the top level (brace_count should be 1)
            if stripped_line == 'interfaces {' and brace_count == 1:
                start_line = i
                break
...
class DrawIOGenerator:
    ...
    def _create_ip_info_elements(self, topology: TopologyBuilder) -> list:
        """Create IP information elements for DrawIO output when IPv4 option is enabled"""
        ip_elements = []
        
        # Calculate the actual diagram height to position IP info below all devices and connections
        max_y = 0
        ...
        # Add padding and ensure minimum position
        ...
        # Collect connection IP info for sorting
        connection_entries = []
        for device_pair, connections in device_pair_connections.items():
            ...
            if len(connections) == 1:
                ...
                if ip1 or ip2:
                    # Format IPs for display using the new helper method
                    ip1_part = device1.extract_ip_part(ip1)
                    ip2_part = device2.extract_ip_part(ip2)
                    
                    ip1_display = f"{dev1}:{port1} {ip1_part}" if ip1_part else f"{dev1}:{port1}"
                    ip2_display = f"{dev2}:{port2} {ip2_part}" if ip2_part else f"{dev2}:{port2}"
                    
                    # Add to list with first device name for sorting
                    connection_entries.append((dev1, f"{ip1_display} <-> {ip2_display}"))
            else:
                ...
                if common_ae1 and common_ae2:
                    ...
                    # Format AE interface with IP and hostnames using the new helper method
                    ae_ip1_part = device1.extract_ip_part(ae_ip1)
                    ae_ip2_part = device2.extract_ip_part(ae_ip2)
                    
                    ae_display1 = f"{dev1}:{ports1}:{common_ae1} {ae_ip1_part}" if ae_ip1_part else f"{dev1}:{ports1}:{common_ae1}"
                    ae_display2 = f"{dev2}:{ports2}:{common_ae2} {ae_ip2_part}" if ae_ip2_part else f"{dev2}:{ports2}:{common_ae2}"
                    
                    # Add to list with first device name for sorting
                    connection_entries.append((dev1, f"{ae_display1} <-> {ae_display2}"))
        
        # Sort connection entries by device name
        connection_entries.sort(key=lambda x: x[0])
        
        # Add sorted entries to connection_lines
        for _, entry in connection_entries:
            connection_lines.append(entry)
        ...
        if len(connection_lines) > 2:  # Only create if there's actual connection info (title + empty line + content)
            ...
            # Create element with light blue background for link IP info (different from interface IP info)
            element = f'''<mxCell id="{element_id}" value="{escaped_connection_text}" style="text;html=1;align=left;verticalAlign=top;whiteSpace=wrap;rounded=0;fontSize=10;fontColor=#000000;fillColor=#e3f2fd;strokeColor=#000000;" vertex="1" parent="1">
            ...
            if not spatial_index.overlaps(bbox):
                ...
                diagram_bottom_y += text_height + 15  # Reduced spacing between elements
        
        # Collect all device interface IP information
            ...
            for interface_name, ip_info in device.interfaces.items():
                if ip_info and interface_name != "lo0":
                    ...
                    if device.device_type == 'switch' or has_neighbor:
                        ...
                        # Extract just the IP part using the new helper method
                        ip_part = device.extract_ip_part(ip_info)
    ...
    try:
        ...
        for device_pair, connections in connection_groups.items():
            ...
            if len(connections) == 1:
                ...
                ip1_part = device1.extract_ip_part(device1.interfaces.get(port1, ""))
                ip2_part = device2.extract_ip_part(device2.interfaces.get(port2, ""))
                
                # Format interface list with hostnames
                ip1_display = f"{dev1}:{port1} {ip1_part}" if ip1_part else f"{dev1}:{port1}"
                ip2_display = f"{dev2}:{port2} {ip2_part}" if ip2_part else f"{dev2}:{port2}"
                
                print(f"{connection_index:3d}. {ip1_display} <-> {ip2_display}")
                connection_index += 1
            else:
                # Multiple connections (AE interfaces)
                for conn in connections:
                ...
                if args.ipv4:
                    ...
                    if common_ae1 and common_ae2:
                        ...
                        # Format AE interface with IP using the new helper method
                        ae_ip1_part = device1.extract_ip_part(ae_ip1)
                        ae_ip2_part = device2.extract_ip_part(ae_ip2)
                        
                        ae_display1 = f"{ports1}:{common_ae1} {ae_ip1_part}" if ae_ip1_part else f"{ports1}:{common_ae1}"
                        ae_display2 = f"{ports2}:{common_ae2} {ae_ip2_part}" if ae_ip2_part else f"{ports2}:{common_ae2}"
